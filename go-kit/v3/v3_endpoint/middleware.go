	...
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, request interface{}) (response interface{}, err error) {
			defer func(begin time.Time) {
				logger.Debug(fmt.Sprint(ctx.Value(v3_service.ContextReqUUid)),
					zap.Any("调用 v3_endpoint LoggingMiddleware", "处理完请求"),
	...
	return func(next endpoint.Endpoint) endpoint.Endpoint {
		return func(ctx context.Context, request interface{}) (response interface{}, err error) {
			...
			if token == "" {
				err = errors.New("请登录")
				logger.Debug(fmt.Sprint(ctx.Value(v3_service.ContextReqUUid)),
					zap.Any("[AuthMiddleware", "token==empty"),
					zap.Error(err))
				return "", err
			}
			...
			if err != nil {
				logger.Debug(fmt.Sprint(ctx.Value(v3_service.ContextReqUUid)),
					zap.Any("[AuthMiddleware]", "ParseToken"),
					zap.Error(err))
				return "", err
			}
